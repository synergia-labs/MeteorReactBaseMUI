...
export default (props: any) => {
  ...
  if (!!newScreenState && validState.indexOf(newScreenState) !== -1) {
    if ((newScreenState === 'view') && !!id) {
      return <ExampleDetailContainer {...props} screenState={newScreenState}
                                     isPrintView={isPrintView}
                                     isFullView={isFullView} id={id}/>;
    } else if (newScreenState === 'edit' && !!id) {
      return <ExampleDetailContainer {...props} screenState={newScreenState}
                                     isPrintView={isPrintView}
                                     isFullView={isFullView} id={id} edit/>;
    } else if (newScreenState === 'create' && !!id) {
      return <ExampleDetailContainer DetailContainer {...props}
                                     screenState={newScreenState}
                                     isPrintView={isPrintView}
                                     isFullView={isFullView} id={id} create/>;
    }
  } else {
    return <ExampleListContainer {...props} isPrintView={isPrintView}
