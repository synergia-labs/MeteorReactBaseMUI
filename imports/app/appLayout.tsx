import React, { createContext, SyntheticEvent, useCallback, useMemo, useState } from 'react';
import {
...
export const AppLayout: React.FC<{ themeOptions: ISysThemeOptions }> = ({ themeOptions }) => {
	...
	const showNotificationHandler = useCallback((props?: IShowNotificationProps) => {
		...
		setShowNotification({
			...showNotification,
			...props,
			close: (event?: SyntheticEvent | Event, reason?: string) =>
				handleCloseNotification(event, reason, props?.onClose),
			open: true
		});
	}, []);
	...
	const showDrawerHandler = useCallback((props?: IShowDrawerProps) => {
		...
		setShowDrawer({
			...
			...props,
			close: (event?: {}, reason?: 'backdropClick' | 'escapeKeyDown') =>
				handleCloseDrawer(event, reason, props?.onClose),
			open: true
		});
	}, []);
	//Fim Show Drawer
	...
	const showModalHandler = useCallback((props?: IShowDialogProps) => {
		...
		setShowModal({
			...
			...props,
			close: (event?: {}, reason?: 'backdropClick' | 'escapeKeyDown') =>
				handleCloseModal(event, reason, props?.onClose),
			open: true
		});
	}, []);
	//Fim Show Modal
	...
	const showWindowHandler = useCallback((props?: IShowDialogProps) => {
		...
		setShowWindow({
			...
			...props,
			close: (event?: {}, reason?: 'backdropClick' | 'escapeKeyDown') =>
				handleCloseWindow(event, reason, props?.onClose),
			open: true
		});
	}, []);
	//Fim Show Window
