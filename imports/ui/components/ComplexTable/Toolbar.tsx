...
export const Toolbar = (props: IToolbarProps) => {
  const { buttonVariant, toolbarOptions, filterIconWidth, openFilterModal } =
    props;

  if (!!toolbarOptions) {
    const hideFilterIcon = useMediaQuery(
      `(min-width:${filterIconWidth ?? 600}px)`
    );

    const { searchFilter, selectColumns, density, exportTable } =
      toolbarOptions;
    return (
