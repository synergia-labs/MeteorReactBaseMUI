...
class SimpleForm extends Component<ISimpleFormProps> {
    ...
    clearForm = () => {
        ...
        this.setState({updateForm:new Date()},()=>{
            self.initFormElements(true);
    ...
    wrapElement = (element, index) => {
        ...
        if (element.props.formType === 'subform' && !!element.props.name) {
            return (<SubFormComponent
                ...
                initialValue={self.docValue&&self.docValue[element.props.name] ? self.docValue[element.props.name] : this.initialValueDefault(self.props.schema[element.props.name])}
        ...
        return (<FieldComponent
            ...
            key={element.props.name ? element.props.name : (`el${index}`)}
            fieldSchema={self.props.schema ? self.props.schema[element.props.name] : undefined}
            initialValue={self.docValue&&self.docValue[element.props.name] ? self.docValue[element.props.name] : (self.props.schema?self.initialValueDefault(self.props.schema[element.props.name]):undefined)}
            reactElement={element}
            setDoc={self.setDoc}
            mode={self.props.mode}
    ...
    validate = () => {
        ...
        if (this.props.schema) {
            ...
            Object.keys(this.fields).forEach((field) => {
                ...
                if (
                    this.props.schema[field] &&
                    this.props.schema[field].type === Date && hasValue(this.getDoc()[field])&&
    ...
    shouldComponentUpdate(nextProps: Readonly<ISimpleFormProps>, nextState: Readonly<{}>, nextContext: any): boolean {
        ...
        if(this.state.error!==nextState.error||this.state.updateForm!==nextState.updateForm) {
            return true;
        }

        if ((!_.isEqual(this.props.doc, nextProps.doc)) || (this.props.mode !== nextProps.mode)) {
            return true
        }
        return false;
    ...
    render() {
        ...
        return (
            <div style={this.props.style || { width: '100%'}}>
                {this.formElements}
                {this.state.error?
                    (<div style={{display:'flex',flexDirection:'row',justifyContent:'flex-end',width:'100%',color:'#FF1010'}}>
