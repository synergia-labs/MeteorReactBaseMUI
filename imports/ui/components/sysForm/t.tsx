...
const SysForm: React.FC<ISysForm> = ({
	...
	const { initialDefaultValues, initialRequiredFields, initialHiddenFields } = useMemo(() => {
		...
		Object.keys(schema).forEach((key) => {
			...
			if (schema[key].visibilityFunction && !schema[key].visibilityFunction?.(doc)) initialHiddenFields.push(key);
	...
	const checkHiddenFieldsVisibilityChanged = useCallback(
		(doc: { [key: string]: any }) => {
			const newHiddenFields = Object.keys(schema).filter(
				(key) => schema[key].visibilityFunction && !schema[key].visibilityFunction?.(doc)
			);
			if (compareArrays(newHiddenFields, hiddenFields)) setHiddenFields(newHiddenFields);
		},
		[hiddenFields]
	);

	const changeDocValue = useCallback(
