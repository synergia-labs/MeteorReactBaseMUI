...
> {
          ...
          <ListItem
            ...
            onClick={() => {
              if (
                ...
                window.open(
                  `/media/${this.props.doc._id}/${item.id}`,
                  item.name
                );
              } else {
  ...
  getConteudoDropzoneEmUpload = () => (
    <div style={uploadFilesStyle.containerStatusUpload}>{"Enviando"}</div>
  );
...
UploadFile.defaultProps = {
  ...
  mensagens: {
    ...
    arquivosRejeitados:
      "Tipos de arquivos permitidos (Tamanho Máximo de 15MB): .xlsx, .xls," +
