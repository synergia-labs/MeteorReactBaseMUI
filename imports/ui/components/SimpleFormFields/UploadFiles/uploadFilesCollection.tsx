...
class UploadFile extends React.Component<IUploadFileProps & IUploadFilesCollection, IUploadFileState> {
  ...
  componentDidUpdate(
    ...
    if (!_.isEqual(this.props.attachments, prevProps.attachments) ||
      ...
      this.fileQueue.forEach((arquivo) => {
        !arquivos.find(
            a => a.name === arquivo.name && a.status === arquivo.status) &&
  ...
  getListReadOnly = () => {
    return (this.state.links.length > 0
              ...
              <ListItem
                  ...
                  onClick={() => {
                    if ((filetype === 'video' || filetype === 'audio') &&
                        this.props.doc &&
                        this.props.doc._id) {
                      window.open(`/media/${this.props.doc._id}/${item.id}`,
                          item.name);
                    } else {
                    ...
                    secondary={
                      <span>
                        ...
                        {item.status && item.status === 'InProgress' ? (
                            ...
                            item.size / 1024 < 1000 ? `${(item.size /
                                1024).toFixed(2)}KB` : `${(item.size /
                                (1024 * 1024)).toFixed(2)}MB`
                        )}
                      </span>
                    }
        ...
        : <div
            style={uploadFilesStyle.containerNoFiles}>{'Não há arquivos'}</div>);
  };
    ...
    return (
        <div data-cy="dropzone" style={{
          ...
          <div style={{
            ...
            <img src="/images/wireframe/dragActive.png" style={{
              maxWidth: 40,
              maxHeight: 40,
              height: '100%',
              width: '100%',
            ...
            <div style={{textAlign: 'center'}}>
              <Typography
                ...
                {isDragActive ? 'Solte o arquivo aqui' : (isMobile
      ...
      uploadInstance.on('uploaded',
          (error: string | null, fileObj: any): void => {
            ...
            if (newFileQueue.length > 0) {
              ...
              const refsName: string = 'fileinput' + this.props.name +
                  this.props.key;
  ...
  render() {
    ...
    return (
        <div key={this.props.name} style={{
          ...appStyle.fieldContainer, ...uploadFilesStyle.containerUploadFiles,
              ...
              <div style={uploadFilesStyle.containerShowFiles}>
                <div style={uploadFilesStyle.subContainerShowFiles}>
                  ...
                  <Dropzone
                      ...
                      ref={(fileInputRef => this['fileinput' + this.props.name +
                      this.props.key] = fileInputRef)}
                  >
