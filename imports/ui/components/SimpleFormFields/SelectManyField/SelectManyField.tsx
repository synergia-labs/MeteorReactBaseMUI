    ...
    const defaultRenderValue = (values: string[] | undefined) => {
        ...
        return (
            <Box style={{ display: 'flex', flexWrap: 'wrap', width: '100%' }}>
                ...
                {maisDeUmValor && (
                    <span>{`${getLabelFromValue(value[0])} + ${values.length - 1}`}</span>
                )}
            </Box>
        );
    };
                        ...
                        {value === '-'
                            ...
                            : value.map((val: unknown, index: number) => {
                                  const objValue = options
                                      ? options.find(
                                            (object: any) => object?.value === val || object === val
                                        )
                                      : hasValue(val) && val;
                                  return (
                                      <Chip
                ...
                {options?.length > 0 && (
                    <Box>
                        <MenuItem
                            ...
                            key={'all'}
                            value={'all'}
                            onClick={handleChangeFlagAllOptions}
                        >
                ...
                {(options || []).map((opt: IOption, index: number) => {
                    return (
                        ...
                        >
                            <Checkbox
                                checked={
                                    (hasValue(value) && value.includes(opt.value || opt)) ||
                                    flagAllOptions
                                }
                            />
