...
export default ({
  ...
  if (!!readOnly) {
    return (<div key={name} style={{display: 'flex', flexDirection: 'column'}}>
      ...
      <TextField variant={'outlined'} key={name} value={value} error={!!error}
                 disabled={!!readOnly} id={name} name={name} {...otherProps}
                 label={null}/>
    </div>);
  }
  const [dateValue, setDateValue] = useState(
      hasValue(value) ? value : new Date());
  ...
  if (otherProps.isNaked) {
    return (
        <InputBase key={name} onChange={onChange} value={value} error={!!error}
                   disabled={!!readOnly} id={name} name={name}
                   label={otherProps.labelDisable
                       ? undefined
                       : label} {...otherProps} />);
  }
  ...
  return (<div key={name} style={{
    ...
    <TextField
        ...
        value={dateValue && dateValue instanceof Date ? dateValue.toISOString().
