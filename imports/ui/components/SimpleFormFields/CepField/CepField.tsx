...
interface IReadOnlyFlag{
	uf?: boolean; estado?: boolean; municipio?: boolean; distrito?: boolean; localidade?: boolean;
	bairro?: boolean; logradouro?: boolean;
}

export const CepField = ({ value, name, screenState, onChange, error, readOnly, isRequired = true, noShowMsgError, permitirEdicao= false}: ICepField ): JSX.Element => {

	const [endereco, setEndereco] = React.useState<IEndereco>(value! ?? { cep: undefined, logradouro: '', bairro: '', estado: '', municipio: '', distrito: ''});
			...
			getEnderecoByCep(cep!, (error: string | null, response: IResponseCep | null) => {
				if (!error) {
					if (response && response.erro === "true") {
						...
						setEndereco({ ...endereco, cep: undefined, logradouro: '', bairro: '', estado: '', municipio: '', distrito: ''});
						handleChangeEndereco(undefined);
						return;
					};
					setPermiteEdicao(false);
					cepError && setCepError(false);
					setEndereco({ cep,logradouro: response?.logradouro, bairro: response?.bairro, estado: response?.uf, municipio: response?.localidade});
					handleChangeEndereco(({ cep, logradouro: response?.logradouro, bairro: response?.bairro, estado: response?.uf, municipio: response?.localidade }));
	...
	React.useEffect(() => {
		...
		if (screenState === "edit" || screenState === "view") {
			if (hasValue(value) && (value?.cep !== cep || value?.logradouro !== logradouro  || value?.bairro !== bairro || value?.estado !== estado || value?.municipio !== municipio || value?.distrito !== distrito)) {
				setEndereco({ cep: value?.cep, logradouro: value?.logradouro, bairro: value?.bairro, estado: value?.estado || '', municipio: value?.municipio || '', distrito: value?.distrito || '' })
			};
		}
	}, [value])
	...
	const {cep, logradouro, bairro, estado, municipio, distrito} = endereco;
		...
		if (isRequired) {
			if (isValidCep && !cepError) {
				if (hasValue(cep) && hasValue(logradouro) && hasValue(bairro) && hasValue(estado) && hasValue(municipio)
					&& (!value || value.cep !== cep || value.logradouro !== logradouro || value.bairro !== bairro || value.estado !== estado || value.municipio !== municipio || value.distrito !== distrito)) {
	...
	const handleChangeEndereco = (endereco: ({ cep?: string, logradouro?: string, bairro?: string, estado?: string, municipio?: string, distrito?: string } | undefined)) => {
	...
	return (
		<>
			...
			<SelectLocalizacao
				...
				value={{ estado: endereco.estado, municipio: endereco.municipio, distrito: endereco.distrito }}
