...
> {
  ...
  componentDidUpdate(
    prevProps: IUploadFileProps,
    prevState: IUploadFileState,
    snapshot
  ) {
    const arquivos = this.props.attachments || [];
  ...
  changeImage = (increment: number) => {
    ...
    if (
      currentIndex + increment >= (this.state.links || []).length &&
      this.state.links[0]
    ) {
        ...
        {this.props.readOnly ? (
          links.length > 0 ? (
            <Container style={{ overflow: "none", padding: 0 }}>
              ...
              >
                {linksSplice.map((link) =>
                  this.getList(link, numCardPage, this.props.readOnly)
                )}
              </div>
            </Container>
          ) : (
            <div style={uploadImagesStyle.containerNoFiles}>
              {"Não há imagens complementares"}
            </div>
          )
        ) : (
...
const UploadMultipleImagesCollection = withTracker(
  (props: IUploadImagesCollection) => {
