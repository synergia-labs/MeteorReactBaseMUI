...
const drawCharacter = (
    character, charObj, layer = null, listOfObjects = null) => {
      ...
      if (typeof subFormat === 'string') {
        ...
        if (subFormat.type === 'path') {
          var path = new Konva.Path({
            ...
            data: subFormat.path,
            draggable: characteres[character].draggable || false,
            fill: subFormat.fill || charObj.color ||
                characteres[character].colors[0] || '#000000',
            opacity: subFormat.fillOpacity ||
                characteres[character].fillOpacity,
          });
          layer.add(path);
          listOfObjects.push(path);
          ...
          const points = subFormat.points.trim().
              replace(/ /g, ',').
              split(',').
              map(n => parseInt(n));
          ...
          var path = new Konva.Line({
            ...
            stroke: subFormat.stroke || subFormat.fill || charObj.color ||
                '#000000',
  ...
  drawAvatar = () => {
    ...
    list.forEach(character => {
      if (this.state[character]) {
        drawCharacter(character, this.state[character], this.defaultLayer,
            this.listOfDefaultLayersObjects);
      }
    });
        ...
        <div>
          ...
          this.props.value != '-' ?
              (
                  ...
                  ? <div
                      style={avatarGeneratorStyle.containerEmptyAvatar}>{'Não há avatar'}</div>
                  : null)
          }
                ...
                >
                  ...
                  {this.state.open ? (
                      <div style={{
                        ...
                        <div id={'avatarContainer' + this.props.name} style={{
                        ...
                        <Button
                            ...
                            onClick={() => {
                              ...
                              list.filter(
                                ...
                                newAvatar[chr].format = formats[Math.floor(
                                    Math.random() * formats.length)];
                                newAvatar[chr].color = colors[Math.floor(
                                  ...
                                  <div
                                      onClick={() => this.setState({
                                        [character]: {
                                          ...
                                          color: characteres[character].colors[characteres[character].colors.indexOf(
                                              this.state[character]
                                                  ? this.state[character].color
                                                  : characteres[character].colors[0]) +
                                          1] || characteres[character].colors[0],
                                        },
                                      })}
                                      style={{
                                        backgroundColor: 'white',
                                  ...
                                  <div key={character} style={{
                                    ...
                                    <div
                                        ...
                                        style={{
                                          ...
                                          'body' && (!this.state[character] ||
                                              !this.state[character].format)
                                              ? '#ffe691'
                                              : '#FFFFFF',
                                          maxWidth: 100,
                                          minWidth: 100,
                                        ...
                                        map(format => {
                                          return (<div
                                              ...
                                              style={{
                                                backgroundColor: this.state[character] &&
                                                this.state[character].format ===
                                                format ? '#ffe691' : '#FFFFFF',
                                                height: 100, width: 100,
                                              }}>
                                            <CharView key={character + format}
                  ...
                  <DialogActions>
                    <Button id="Fechar Avatar" autoFocus onClick={this.onClose}
                            variant={'outlined'} color={'#74B9FF'}
                            style={{borderColor: '#74B9FF', color: '#74B9FF'}}>
                      {'Fechar'}
                    </Button>
                    <Button id="Salvar Avatar" onClick={this.onSave}
                            variant={'contained'} color={'#74B9FF'} style={{
                      borderColor: '#74B9FF',
