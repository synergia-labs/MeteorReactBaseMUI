...
export default ({name, label, value, onChange, readOnly, error, ...otherProps}: IBaseSimpleFormComponent) => {
    ...
    const [dateValue, setDateValue] = useState(value && value instanceof Date && hasValue(value) ? new Date(value) : undefined);
    ...
    const handleChangeFormat = (evt) => {
        onChange({name: formatSchemaName, target: {formatSchemaName, value: evt.target.value}})
        setFormat(evt.target.value)
    }

    const handleChangeOutro = (evt) => {
        onChange({name: outroSchemaName, target: {outroSchemaName, value: evt.target.value}})
    ...
    return (
        <div style={{display: 'flex', flexDirection: isMobile ? 'column' : 'row'}}>
            <div style={{display: 'flex', flexDirection: 'column', maxWidth: isMobile ? '100%': '40%', minWidth: isMobile ? '100%': '40%', ...appStyle.fieldContainer}}>
                <SimpleLabelView label={'Formato Data ' + otherProps.labelAddOn}/>
                <Select
            ...
            <div key={name} style={{display: 'flex', flexDirection: 'column', ...appStyle.fieldContainer}}>
                <SimpleLabelView label={'Data ' + otherProps.labelAddOn}/>
                <TextField
                    variant={'outlined'}
            ...
            {format === 'outrosAno' &&
                <div style={{display: 'flex', flexDirection: isMobile? 'column' : 'row', width: '100%'}}>
                    <div style={{display: 'flex', flexDirection: 'column', width: isMobile ? '100%': '40%', ...appStyle.fieldContainer}}>
                        <SimpleLabelView label={'Outros'}/>
                        <TextField
                            variant={'outlined'}
                            key={'data_outro'}
                    ...
                    <div style={{display: 'flex', flexDirection: 'column', maxWidth: isMobile ? '100%': '30%', ...appStyle.fieldContainer}}>
                        <SimpleLabelView label={'Ano ' + otherProps.labelAddOn}/>
                        <TextField
                            variant={'outlined'}
                            key={'outro_ano'}
