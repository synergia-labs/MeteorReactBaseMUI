...
interface ISelectMunicipioDistritoField extends IBaseSimpleFormComponent {
	...
	showRadios: boolean,
	estado: String,
	onChangeSelectedValue: (fieldTarget: React.ChangeEvent<HTMLInputElement> | {
		name: String;
		target: Object;
	}, field: Object) => void;
	handleOnChange: (event: React.SyntheticEvent<Element, Event>, selectedValue: any, reason: AutocompleteChangeReason, details?: AutocompleteChangeDetails<unknown> | undefined) => void;
...
export const SelectMunicipioDistritoField = ({
	...
	const handleOnChange = (evt: React.SyntheticEvent<Element, Event>, selectedValue: any) => {
		if (!selectedValue) {
			onChangeSelectedValue(
	...
	if (readOnly) {
		return (
			<div style={{ width: '100%', display: 'flex', flexDirection: 'column', ...appStyle.fieldContainer }}>
	...
	const initialValue = !!value && Object.keys(value).length > 0 ? `${value.municipio} ${value.distrito ? ' - ' + value.distrito : ''}` : undefined;
	const fieldProps = {
	...
	return (
		<div style={{ width: '100%', display: 'flex', flexDirection: 'column', ...appStyle.fieldContainer }} key={name + initialValue ? 'hasValue' : 'noValue'}>
			...
			{initialValue ? (
				<Autocomplete
					...
					options={localidades.filter(function (entry) {
						return entry.u == estado;
					}).map(l => ({
						value: JSON.stringify({ municipio: l.m, distrito: l.d ? l.d : null }),
						label: `${l.m} ${l.d ? ' - ' + l.d : ''}`,
					}))}
				...
				<Autocomplete
					...
					getOptionLabel={(option: any) => (option.label ? option.label : '')}
					style={style || {
						width: '100%', backgroundColor: '#FFF', height: 38,
					}}
					onChange={handleOnChange}
					error={!!error}
					disabled={!!readOnly}
