...
interface ISelectLocalizacao extends IBaseSimpleFormComponent {
	...
	isSearch : boolean,
  onChange: (fieldTarget: React.ChangeEvent<HTMLInputElement> | IEventLocalizacao | {name: String; target: Object}, field?: Object) => void;
...
export const SelectLocalizacao = React.memo(({naoObrigatorio, onChange, value, name, error, style, help, readOnly, label, isSearch, ...otherProps }: ISelectLocalizacao) => {
	...
	if (readOnly) {
		return (
			...
			>
				{label && !otherProps.rounded ? <SimpleLabelView label={label = 'Estado/Cidade'} help={help} /> : null}
