...
import { IEventLocalizacao } from "../CepField/CepField";
import { Theme } from "@mui/material";

interface ISelectLocalizacao extends IBaseSimpleFormComponent {
  ...
  onChange: (
    fieldTarget:
      ...
      | IEventLocalizacao
      | { name: String; target: Object },
    field?: Object
  ) => void;
  definirEstado: (e: React.ChangeEvent<HTMLInputElement>) => void;
  handleChange: (localizacao: { [loc: string]: any } | undefined) => void;
  definirMunicipio: (e: React.ChangeEvent<HTMLInputElement>) => void;
  [otherPropsKey: string]: any;
}

export interface IDefinirMunicipio extends React.ChangeEvent<HTMLInputElement> {
...
export const SelectLocalizacao = React.memo(
  ({
    naoObrigatorio,
    onChange,
    value,
    ...
    const definirEstado = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleChange = (localizacao: { [loc: string]: any } | undefined) => {
      if (
        localizacao &&
        Object.keys(localizacao).find((loc) => !!localizacao[loc])
      ) {
        onChange(
    ...
    const definirMunicipio = (e: IDefinirMunicipio) => {
    ...
    useEffect(() => {
      ...
      if (
        ...
        if (isSearch) handleChange(localizacao);
        else if (estado && municipio) handleChange(localizacao);
        else handleChange(undefined);
      }
    }, [estado, municipio, distrito]);

    useEffect(() => {
      if (value && value.estado !== estado) setEstado(value.estado);
      if (value && value.municipio !== municipio) setMunicipio(value.municipio);
      if (value && value.distrito !== distrito) setDistrito(value.distrito);
      if (!value) {
        setEstado(undefined);
        setMunicipio(undefined);
        setDistrito(undefined);
    ...
    return (
      <div
        style={{ display: "flex", flexDirection: isSmall ? "column" : "row" }}
      >
        ...
        <div style={{ width: isSmall ? "100%" : "75%" }}>
          <SelectMunicipioDistritoField
            ...
            //@ts-ignore
            onChangeSelectedValue={definirMunicipio}
            rounded={otherProps.rounded}
          />
        </div>
      </div>
