  ...
  if (readOnly) {
    ...
    return (
        <div key={name} style={{
          ...
          <TextField value={value ? (value.municipio +
              (value.distrito ? (' - ' + value.distrito) : '')) : undefined}
                     disabled={true} label={null} variant={'outlined'}/>
        </div>
    );
  }
        ...
        <Autocomplete
            ...
            options={localidades.map(l => ({
              value: JSON.stringify(
                  {estado: l.u, municipio: l.m, distrito: l.d}),
              label: l.u + ' - ' + l.m + (l.d ? ' - ' + l.d : ''),
            }))}
            ...
            disabled={!!readOnly}
            value={!!value && Object.keys(value).length > 0 ? JSON.stringify(
                value) : undefined}
            inputValue={!!value && Object.keys(value).length > 0
                ? value.estado + ' - ' + value.municipio +
                (value.distrito ? ' - ' + value.distrito : '')
                : undefined}
            renderInput={params => <TextField id={name}
                                              error={!!error} {...params}
                                              variant={'outlined'}
                                              label={null}/>}
