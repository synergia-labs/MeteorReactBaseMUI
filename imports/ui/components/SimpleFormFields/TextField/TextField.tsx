...
export default ({name,label,value,onChange,readOnly,error,help, style, ...otherProps}:IBaseSimpleFormComponent)=>{
    ...
    return (<div key={name} style={{display: 'flex',flexDirection: 'column',...appStyle.fieldContainer}}>
        ...
        <TextField style={style ? style:{ backgroundColor: 'white', borderColor: '#f2f2f2'}} variant={'outlined'} InputProps={otherProps.rounded || otherProps.field?{root: classes.root}:undefined} key={name} onChange={onChange} value={value} error={!!error} disabled={!!readOnly} id={name} name={name} label={otherProps.rounded?label:null} {...otherProps}/>
    </div>);
}
