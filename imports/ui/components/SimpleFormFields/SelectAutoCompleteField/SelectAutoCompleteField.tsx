...
export const getLocalidade = () => {
    const newLocalidade = bemculturalLocalidade.map((l) => {
        const uf = l.municipio.microrregiao && l.municipio.microrregiao.mesorregiao && l.municipio.microrregiao.mesorregiao.UF ? l.municipio.microrregiao.mesorregiao.UF.sigla : (
            l['regiao-imediata'] && l.municipio['regiao-imediata']['regiao-intermediaria'] && l.municipio['regiao-imediata']['regiao-intermediaria'].UF ? l.municipio['regiao-imediata']['regiao-intermediaria'].UF.sigla : null
        );
...
export default ({
    ...
    const municipios = value.estado && value.estado.id ? bemculturalLocalidade.filter(x =>
        (x.municipio.microrregiao.mesorregiao.UF.id === value.estado.id && x.municipio.nome === x.nome),
    ) : bemculturalLocalidade.filter(x => x.municipio.nome === x.nome);

    const distritos = municipio => (municipio && municipio.nome ? bemculturalLocalidade.filter(
    ...
    const onChangeOrigem = (evt: React.BaseSyntheticEvent) => {
        onChange(
            {name, target: {name, value: {...value, identificada: evt.target.value === 'Sim'}}},
    ...
    if (readOnly) {
        return (<div key={name}>
            {value ?
                <div
                    style={{width: '100%', display: 'flex', flexDirection: 'column', ...appStyle.fieldContainer}}
                    key={name}
                >
                    <SimpleLabelView label={label}/>
                    {value.distrito && <SimpleLabelView value={value.distrito.nome}/>}
                    {value.municipio && <SimpleLabelView value={value.municipio.nome}/>}
                    {value.estado && <SimpleLabelView value={value.estado.nome}/>}
                </div>
    ...
    return (
        <div>
            {showRadios && mode != 'filter' && <RadioButtonField
                label={'Identificada?'}
            ...
            <div>
                {estadoOn && <Autocomplete
                    ...
                    defaultValue={value && value.estado ? opcoesEstados.find(x => x.nome === value.estado.nome) : opcoesEstados[0]}
                />}
                ...
                <Autocomplete
                    ...
                    error={!!error}
                    disabled={!!readOnly}
                    renderInput={params => <TextField {...params} variant={'outlined'} label={null}/>}
                    defaultValue={value && value.municipio ? bemculturalLocalidade.find(x => x.municipio.nome === value.municipio.nome) : bemculturalLocalidade}
                />
                ...
                <Autocomplete
                    ...
                    defaultValue={value && value.distrito ? bemculturalLocalidade.find(x => x.nome === value.distrito.nome) : {nome: ''}}
                />
                }
            </div>
            }
