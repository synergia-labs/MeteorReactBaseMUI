...
export const getLocalidade = () => {
  const newLocalidade = bemculturalLocalidade.map((l) => {
    const uf =
      l.municipio.microrregiao &&
      l.municipio.microrregiao.mesorregiao &&
      l.municipio.microrregiao.mesorregiao.UF
...
export default ({
  ...
  const distritos = (municipio) =>
    municipio && municipio.nome
      ? bemculturalLocalidade.filter(
          (x) =>
            x.municipio.nome === value.municipio.nome &&
            x.nome !== value.municipio.nome
        )
      : [];
      ...
      <div key={name}>
        {value ? (
          ...
          >
            ...
            {value.municipio && (
              <SimpleLabelView value={value.municipio.nome} />
            )}
            {value.estado && <SimpleLabelView value={value.estado.nome} />}
          </div>
        ) : (
          <div style={selectAutoCompleteStyle.containerEmptyItens}>
            {"Não identificada"}
          </div>
        )}
      </div>
        ...
        <div>
          {estadoOn && (
            <Autocomplete
              ...
              defaultValue={
                value && value.estado
                  ? opcoesEstados.find((x) => x.nome === value.estado.nome)
                  : opcoesEstados[0]
              }
            />
          )}
          ...
          <Autocomplete
            ...
            disabled={!!readOnly}
            renderInput={(params) => (
              <TextField {...params} variant={"outlined"} label={null} />
            )}
            defaultValue={
              value && value.municipio
                ? bemculturalLocalidade.find(
                    (x) => x.municipio.nome === value.municipio.nome
                  )
          ...
          {hasValue(value.municipio) && distritoOn && municipioOn && (
            <Autocomplete
              ...
              disabled={!!readOnly}
              renderInput={(params) => (
                <TextField {...params} label={"Distrito (Opcional)"} />
              )}
              defaultValue={
                value && value.distrito
                  ? bemculturalLocalidade.find(
                      (x) => x.nome === value.distrito.nome
                    )
                  : { nome: "" }
              }
            />
