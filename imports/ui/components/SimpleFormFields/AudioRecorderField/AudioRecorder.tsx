    ...
    recorder.ondataavailable = (e: any) => {
      ...
      reader.onloadend = () => {
        ...
        onChange({name, target: {name, value: reader.result}},
            {name, value: reader.result}); // You can upload the base64 to server here.
      };
  ...
  if (!!readOnly) {
    return (<div key={name}>
      ...
      {hasValue(value) && value != '-' ?
          <p>
            <audio src={value} controlsList={'nodownload'} controls="controls"
                   autobuffer="autobuffer"
                   style={audioRecorderStyle.buttonOptions}/>
          </p>
          : <div
              style={audioRecorderStyle.containerEmptyAudio}>{'Não há aúdio'}</div>}
    </div>);
  }

  return (
      <div key={name} style={error
          ? audioRecorderStyle.containerRecordError
          : audioRecorderStyle.containerRecord}>
        <SimpleLabelView label={label}/>
        <div style={audioRecorderStyle.subContainerRecord}>
          {!hasValue(value) || value == '-' ?
              <span style={audioRecorderStyle.subContainerRecord}>
              <Fab color="secondary" onClick={handleRecordAudio}
                   value={values.recordButton} aria-label="record"
                   className="record" id="record"
                   disabled={!values.recordButton}
                   style={audioRecorderStyle.buttonOptions}>
                  <KeyboardVoiceIcon/>
              </Fab>

              <Fab color="secondary" aria-label="play" className="stop"
                   onClick={handleStopRecordAudio} value={values.stopButton}
                ...
                {values.recordButton ? '' :
                    <Fab variant="outlined" color="#5a9902"
                         style={audioRecorderStyle.buttonCountOptions}>
            ...
            <audio src={value} controlsList={'nodownload'} controls="controls"
                   autobuffer="autobuffer"
                   style={audioRecorderStyle.buttonOptions}/>
              <Fab color="secondary" aria-label="delete" className="delete"
                   style={audioRecorderStyle.buttonOptions}
                   onClick={deleteAudio}>
                <DeleteIcon/>
              </Fab>
