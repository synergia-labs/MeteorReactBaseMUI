...
export default ({name, label, value, onChange, readOnly, error}: IBaseSimpleFormComponent) => {
    ...
    if (readOnly) {
        return (<div key={name}>
            ...
            {hasValue(value) && value != '-' ?
                <p>
                    <audio
                        src={value} controlsList={'nodownload'} controls="controls" autobuffer="autobuffer"
    ...
    return (
        <div key={name} style={error ? audioRecorderStyle.containerRecordError : audioRecorderStyle.containerRecord}>
            ...
            <Fab
                color="secondary" onClick={handleRecordAudio} value={values.recordButton} aria-label="record"
                className="record" id="record" disabled={!values.recordButton}
                style={audioRecorderStyle.buttonOptions}
            >
              <KeyboardVoiceIcon/>
            </Fab>
            ...
            <Fab
                color="secondary" aria-label="play" className="stop" onClick={handleStopRecordAudio}
                value={values.stopButton} disabled={!values.stopButton} style={audioRecorderStyle.buttonOptions}
            >
              <StopIcon/>
            </Fab>
            ...
            />
            <Fab
                color="secondary" aria-label="delete" className="delete" style={audioRecorderStyle.buttonOptions}
                onClick={deleteAudio}
            >
              <DeleteIcon/>
            </Fab>
