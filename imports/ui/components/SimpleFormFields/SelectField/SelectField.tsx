...
export default ({
      ...
      createStyles({
        ...
        input: {
          ...
          transition: theme.transitions.create(
              ['border-color', 'border-size', 'box-shadow']),
  ...
  if (readOnly) {
    ...
    let objValue = options ? options.find(
        object => (object.label === value || object === value)) : hasValue(
        value) && value;
    if (multiple) {
      ...
      return (
          <div key={name} style={{
            width: '100%',
            display: 'flex',
            flexDirection: 'column', ...(otherProps.style ||
    ...
    return (
        <div key={name}
          ...
          <TextField id={name} key={name}
                     value={(value === '-' ? '-' : ((objValue && objValue.label
                         ? objValue.label
                         : (objValue || null))))} disabled={true}
  ...
  const onChangeSelect = (event) => {
    if (!readOnly) {
      onChange({name, target: {name, value: event.target.value}},
      ...
      <FormControl variant="outlined" key={name} style={{
        ...
        >
          ...
          {(options || []).map(opt =>
              ...
              >
                {multiple && <Checkbox
                    checked={!!value && value.includes(opt.value || opt)}/>}
