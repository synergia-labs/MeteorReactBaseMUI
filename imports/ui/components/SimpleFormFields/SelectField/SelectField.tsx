...
export default ({
  ...
  const options =
    otherProps.options || (schema && schema.options ? schema.options : []);
  ...
  if (readOnly) {
    ...
    if (multiple) {
      objValue =
        hasValue(value) && renderValue ? renderValue(value) : undefined;
  ...
  return (
    ...
    >
      ...
      {label && !!otherProps.rounded ? (
        <InputLabel style={{ backgroundColor: "white", marginLeft: 4 }}>
          {label}
        </InputLabel>
      ) : null}
