...
export default ({
    ...
    if (readOnly) {
        ...
        return (
            <div key={name}
                ...
                <TextField id={name} key={name}
                           value={(value==='-'?'-':((objValue && objValue.label ? objValue.label : (objValue || null))))} disabled={true}
                           label={otherProps.rounded ? label : null} variant={"outlined"}/>
            </div>
        );
    }
    ...
    return (
        <FormControl variant="outlined" key={name} style={{
            ...appStyle.fieldContainer,
            display: 'flex',
            flexDirection: 'column',
            width: '100%', ...(otherProps.style || {})
        }}>
            {label && !otherProps.rounded ? <SimpleLabelView label={label} help={help}/> : null}
            ...
            <Select
                ...
                style={{...(style ? style : {
                    borderColor: '#f2f2f2'
                }), ...{border: error ? '1px solid #ff0000' : 'undefined', borderRadius: error ? '4px' : undefined,}}}
