...
const SimpleExportCSV = ({filter,options,publishName,api,filename}: any) => {
    ...
    const showSnackBar = () => (
        ...
        >
            <Alert
                id={'message-id'} icon={false} arialabel={'message-id'} onClose={() => setShowLoading(false)}
                color={'info'} severity={'info'} elevation={6} variant="filled"
            >
                <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
                    <div>
                        {'Aguarde enquanto o CSV está sendo gerado!'}
                    </div>
