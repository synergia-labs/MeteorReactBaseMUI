        ...
        <TableRow>
          {headCells.map(headCell => (
              <TableCell
                  ...
                  (headCell.sortField || headCell.field) ? order : false}
              >
                {disabledOrder ?
                    headCell.label
                    :
                    <TableSortLabel
                        active={orderBy ===
                        (headCell.sortField || headCell.field)}
                        direction={orderBy ===
                        (headCell.sortField || headCell.field) ? order : 'asc'}
...
export default function SimpleTable({
  ...
  const handleRowClick = (
      id: string, doc: object) => (event: React.SyntheticEvent) => {
    if (onClick) {
    ...
    if (type === 'image') {
      ...
      return <div style={{display: 'flex', flexDirection: 'column'}}>
        {Object.keys(data).map(key => {
          return <div
              style={{lineHeight: 0.8, marginBottom: 10}}>{data[key]}<br/><span
              style={{fontSize: 9, color: '#777'}}>{key}</span></div>;
        })}
      </div>;
    } else if (type === 'boolean') {
      return data ? <CheckIcon style={{width: '15px'}}/> : <CloseIcon
          style={{width: '15px'}}/>;
    } else if (type === 'html') {
      return Array.isArray(data) ? data.map(
          d => <div dangerouslySetInnerHTML={{__html: d}}/>) : <div
      ...
      const schemaData = data && schema[colName] && schema[colName].options &&
          schema[colName].options.find(
              object => (object.value === data || object === data));
      return (schemaData && (schemaData.label || schemaData.value) || data);
    }
    return null;
  };

  const cols = Array.isArray(schema) ? schema.map(
          col => ({...col, type: getType(col)}))
      : Object.keys(schema).
          map(field => ({
  ...
  const newDate = !data ? null : stableSort(data,
      getComparator(order, orderBy));

  const filterList = {};
  const tableBody = ((filter ? (newDate.filter(
      d => d[filterByField] === filter)) : newDate).map((row, index) => {
    if (filterByField) {
        ...
        >
          {cols.map(col => (<TableCell
              ...
              style={{
                ...
                textAlign: (col.type === 'image' || col.type === 'dom')
                    ? 'flex-start'
                    : undefined,
      ...
      <div style={{display: 'flex'}}>
        {filterByField ? (
            <div style={{
              ...
              {'Filtro: '}{filter ? (
                <span style={{fontWeight: 'bold'}}>{filter} <Button
                    style={{color: '#F00'}}
                    onClick={() => setFilter(null)}>{'X'}</Button></span>) : (
                <Select value={filter} onChange={evt => (!evt.target.value
                    ? setFilter(null)
                    : setFilter(evt.target.value))}>
                  <MenuItem key={'NoFilter'}
                            value={null}>{'Sem Filtro'}</MenuItem>
        ...
        <Table className={'mobileTable'}
               style={{...simpleTableStyle.tableBox, ...styles}}>
          {/* <TableHead> */}
          {/*    <TableRow > */}
          {/*        {cols.map(col=>{ */}
