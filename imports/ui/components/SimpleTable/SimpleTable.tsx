...
const descendingComparator = (
    a: { [x: string]: number; noOrder: any },
    b: { [x: string]: number; noOrder: any },
...
const getComparator = (order: string, orderBy: any) =>
    order === 'desc'
        ? (a: { [x: string]: number; noOrder: any }, b: { [x: string]: number; noOrder: any }) =>
...
export const SimpleTable = React.memo(
    ({
            ...
            if (type === 'select') {
                const value =
                    schema && schema[colName] && schema[colName].options
                        ? schema[colName].options.find((d) => d.value === data || d === data)
                        : data;
        ...
        const filterList = {};
        const tableBody = (
            filter ? newDate.filter((d) => d[filterByField] === filter) : newDate
        ).map((row, index) => {
            if (filterByField) {
