...
export default class Signup extends React.Component {
  ...
  handleSubmit(doc) {
    ...
    userprofileApi.insertNewUser({email, username: email, password},
        (err, r) => {
          if (err) {
          ...
          <SimpleForm onSubmit={this.handleSubmit}>
            ...
            <div style={signupStyle.containerButtonOptions}>
              <Button color={'primary'} variant={'outlined'}
                      submit>{'Cadastrar'}</Button>
            </div>

          </SimpleForm>
          <div style={signupStyle.containerRouterSignIn}>
            Já tem uma conta? Faça login clicando <Link to="/signin"
                                                        color={'secondary'}>aqui</Link>
          </div>
        </Container>
    );
  }
