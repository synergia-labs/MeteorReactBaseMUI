...
import SelectField
  from '/imports/ui/components/SimpleFormFields/SelectField/SelectField';
import CheckBoxField
  from '/imports/ui/components/SimpleFormFields/CheckBoxField/CheckBoxField';
...
export default function DevUtils() {
  ...
  const getField = (doc, schema, field, imports, formFields) => {
    if (schema[field].isImage) {
      ...
      formFields.push(
          `<SimpleImageUploadBase64 label={'${schema[field].label}'}  name='${field}' />`);
      ...
      formFields.push(
          `<AudioRecorder label={'${schema[field].label}'}  name='${field}' />`);
      ...
      formFields.push(
          `<UploadFilesCollection label={'${schema[field].label}'}  name='${field}' doc={${doc.api}Doc}/>`);
      ...
      formFields.push(
          `<GoogleApiWrapper label={'${schema[field].label}'}  name='${field}'/>`);
      ...
      formFields.push(
          `<SelectField options={'${schema[field].options}'} label={'${schema[field].label}'}  name='${field}'/>`);
      ...
      formFields.push(
          `<TextMaskField mask={'${schema[field].mask}'} label={'${schema[field].label}'}  name='${field}'/>`);
      ...
      formFields.push(
          `<TextField label={'${schema[field].label}'}  name='${field}'/>`);
      ...
      formFields.push(
          `<DatePickerField label={'${schema[field].label}'}  name='${field}'/>`);
      ...
      formFields.push(
          `<ChipInput label={'${schema[field].label}'}  name='${field}'/>`);
    } else if (schema[field].type === Object) {
      if (schema[field].subSchema) {
        formFields.push(
            `<FormGroup formType={'subform'} label={'${schema[field].label}'}  name='${field}>`);
        ...
        formFields.push(
            `<CheckBoxFieldlabel checksList={'${schema[field].checksList}'} label={'${schema[field].label}'} name='${field}'/>`);
      }
    } else if ((schema[field].type).toString() === ([Object]).toString()) {
      if (schema[field].subSchema) {
        formFields.push(
            `<FormGroup formType={'subformArray'} label={'${schema[field].label}'}  name='${field}>`);
        Object.keys(schema[field].subSchema).forEach(subField => {
          getField(doc, schema[field].subSchema, subField, imports, formFields);
        });
        formFields.push(`</FormGroup`);
      ...
      formFields.push(
          `<ToggleSwitchField label={'${schema[field].label}'}  name='${field}'/>`);
    }
  };
      ...
      <div className={classes.root}>
        ...
        <TabPanel value={value} index={0}>
          ...
          >
            ...
            <Button key={'gerar'} color={'primary'} variant="contained"
                    submit="true">
              {'Gerar'}
            </Button>
          </SimpleForm>
          {formResult ? (
              <div style={{
                ...
                <h3>Imports</h3>
                {formResult && formResult.imports ? formResult.imports.map(
                    val => <div style={{width: '100%'}}>{val}</div>) : null}

                <h4>Fields</h4>
                {formResult && formResult.formFields
                    ? formResult.formFields.map(
                        val => <div style={{width: '100%'}}>{val}</div>)
                    : null}
              </div>
          ) : null}
