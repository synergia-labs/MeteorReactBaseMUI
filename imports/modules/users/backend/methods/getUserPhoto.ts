...
class GetUserPhotoCallMethod extends MethodBase<UsersServer, Omit<GetFileUrlType, "type">, string | undefined> {
	constructor() {
		super({
			name: enumUsersRegisterMethods.getUserPhoto,
	...
	async action(
		...
		const user = await serverInstance.getMongo().findOneAsync({
			_id: _id
		});
		if (!user) this.generateError({ key: "notFound", code: "404" }, _context);
		if (!user?.profile?.photo) return undefined;
		if (isValidLink(user?.profile?.photo)) {
			const { _id, path } = await storageServer.uploadImage({
				archive: await fetchImageMetadata(user?.profile?.photo)
			});
			await serverInstance.getMongo().updateAsync({ _id: user._id }, { $set: { "profile.photo": _id } });
			return path;
		}
