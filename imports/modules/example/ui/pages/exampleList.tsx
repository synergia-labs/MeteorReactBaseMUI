...
const ExampleList = (props: IExampleList) => {
  ...
  const {examples, navigate, remove, showDialog, onSearch, total, loading, setFilter, clearFilter, setPage,
  ...
  return (
      <PageLayout
          ...
          actions={[]}
      >
        <SearchDocField api={userprofileApi}
        ...
        />
        <SimpleTable
            schema={_.pick({...exampleApi.schema, nomeUsuario: {type: String, label: 'Criado por'}},
...
export const ExampleListContainer = withTracker((props: IDefaultContainerProps) => {
  ...
  const subHandle = exampleApi.subscribe('exampleList', filter, {sort, limit, skip});
