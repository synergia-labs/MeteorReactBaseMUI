...
function validateSocialLoginAndUpdateProfile(userProfile, user, serviceName) {
  ...
  if (!userProfile) {
    ...
    if (user.services && !!user.services.google &&
      ...
      updateUserProfileImageFromURL(userProfileID,
          user.services.google.picture);
    }
    ...
    if (!userProfile.photo) {
      if (user.services && !!user.services.google &&
          user.services.google.picture) {
        updateUserProfileImageFromURL(userProfile._id,
            user.services.google.picture);
      }
    }
  }
...
Meteor.startup(() => {
  ...
  Accounts.onLogin((params) => {
    console.log('OnLogin:', params && params.user ? {
      _id: params.user._id,
  ...
  Accounts.validateLoginAttempt(({user, allowed}) => {
    ...
    if (!user.emails[0].verified) {
      throw new Meteor.Error('Email ñao verificado',
