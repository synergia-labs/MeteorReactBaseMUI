...
class PersistentMinimongoStorage {
  constructor(collectionName, collectionInstance) {
    ...
    getControlStoreData = () => this.controlStoreData || { removedDocs: [], updatedDocs: [], syncHistory: [], lastClientSync: new Date() }
    ...
    getDocs = (filter = {}, callback = () => {
      ...
      const self = this;
      const matchFilter = (o1, o2) => Object.keys(o1).map(k => _.isEqual(o1[k], o2[k]).filter(o => !!o).length === Object.keys(o1).length);
      this.updateKeys((e, r) => {
        const result = [];
        if (r) {
          self.list.forEach(async (key, i) => {
      ...
      (controlStoreData.removedDocs || []).forEach((docId) => {
        removeDocFunc({ _id: docId }, (e, r) => {
          if (!e) {
            ...
            self.updateSyncHistory({ date: new Date(), type: 'remove', status: 'error', error: e, docId });
          }
        });
      });
    }
