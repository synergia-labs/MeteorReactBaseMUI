...
class AttachmentsCollection {
    ...
    serverRemoveByDocId = (docId) => {
        if (Meteor.isServer) {
            ...
            const attachmentDocs = this.attachments.collection.find({'meta.docId': docId}).fetch();
    ...
    serverSaveCSVFile = async (file) => {
        if (Meteor.isServer) {
            ...
            const fs = require('fs').promises;
            const fileSave = await fs.writeFile(`${uploadPaths}/${nameFile}`
                , file);
